...
import type { FileSystemService, GitService, LoggerService } from './services/types.ts';
...
export async function validateAndMergeConfig(partialConfig: PartialWTConfig, repoInfo: RepositoryInfo): Promise<WTConfig> {
  const defaultServices = createServiceContainer();
  const configOps = new ConfigOperations(defaultServices.fs, defaultServices.git, defaultServices.logger);
  return configOps.validateAndMergeConfig(partialConfig, repoInfo);
}
...
export async function saveConfig(repoInfo: RepositoryInfo, config: WTConfig): Promise<void> {
  const defaultServices = createServiceContainer();
  const configOps = new ConfigOperations(defaultServices.fs, defaultServices.git, defaultServices.logger);
  return configOps.saveConfig(repoInfo, config);
}
...
export async function updateConfigValue(
  ...
  const defaultServices = createServiceContainer();
  const configOps = new ConfigOperations(defaultServices.fs, defaultServices.git, defaultServices.logger);
  return configOps.updateConfigValue(repoInfo, key, value);
}
...
/**
 * Service-based configuration operations class for dependency injection
 */
export class ConfigOperations {
  ...
  async detectDefaultWorktreeDir(repoInfo: RepositoryInfo): Promise<string> {
    try {
      this.logger.debug('Detecting default worktree directory');
      
      // Get list of all worktrees
      const worktreeList = await this.git.executeCommand(repoInfo.gitDir, ['worktree', 'list', '--porcelain']);
      
      if (!worktreeList) {
        this.logger.debug('No worktrees found, using current directory');
        return './';
      }
      ...
      if (worktreePaths.length === 0) {
        this.logger.debug('No valid worktree paths found, using current directory');
        return './';
      }
      ...
      if (worktreePaths.length === 1) {
        ...
        if (worktreeParent) {
          const relativePath = relative(repoInfo.rootDir, worktreeParent) || './';
          this.logger.debug(`Single worktree detected, using parent: ${relativePath}`);
          return relativePath;
        }
        return './';
      }
      ...
      if (commonParent) {
        ...
        this.logger.debug(`Multiple worktrees detected, using common parent: ${result}`);
        return result;
      }
      return './';
      ...
      this.logger.debug(`Git command failed during worktree detection: ${message}, falling back to current directory`);
      return './';
    }
  }

  async createDefaultConfig(repoInfo: RepositoryInfo): Promise<WTConfig> {
    const defaultWorktreeDir = await this.detectDefaultWorktreeDir(repoInfo);
  ...
  async loadConfig(repoInfo: RepositoryInfo): Promise<WTConfig> {
    ...
    try {
      ...
      return await this.validateAndMergeConfig(parsedConfig, repoInfo);
      
    } catch (error) {
      if (error instanceof SyntaxError) {
        const message = `Invalid JSON in config file ${configPath}: ${error.message}`;
        this.logger.error(message);
        throw new ConfigError(message, EXIT_CODES.FILESYSTEM_ERROR);
      }
      
      // If file doesn't exist, return default config
      if ((error as NodeJS.ErrnoException).code === 'ENOENT') {
        this.logger.debug('Config file not found, using default configuration');
        return await this.createDefaultConfig(repoInfo);
      }
      
      // Re-throw other filesystem errors
      const message = `Failed to read config file ${configPath}: ${(error as Error).message}`;
      this.logger.error(message);
      throw new ConfigError(message, EXIT_CODES.FILESYSTEM_ERROR);
    }
  }

  async validateAndMergeConfig(partialConfig: PartialWTConfig, repoInfo: RepositoryInfo): Promise<WTConfig> {
    const config: WTConfig = await this.createDefaultConfig(repoInfo);
  ...
  async saveConfig(repoInfo: RepositoryInfo, config: WTConfig): Promise<void> {
    const configPath = join(repoInfo.rootDir, CONFIG_FILE_NAME);
    this.logger.debug(`Saving config to: ${configPath}`);
    
    try {
      const configContent = JSON.stringify(config, null, 2);
      await this.fs.writeFile(configPath, configContent, 'utf-8');
      this.logger.debug('Config saved successfully');
    } catch (error) {
      const message = `Failed to save config file ${configPath}: ${(error as Error).message}`;
      this.logger.error(message);
      throw new ConfigError(message, EXIT_CODES.FILESYSTEM_ERROR);
    }
  }
  ...
  async updateConfigValue(
    ...
    const currentConfig = await this.loadConfig(repoInfo);
    ...
    const validatedConfig = await this.validateAndMergeConfig(currentConfig, repoInfo);
    
    // Save the updated configuration
    await this.saveConfig(repoInfo, validatedConfig);
