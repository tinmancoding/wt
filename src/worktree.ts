...
/**
 * Handles worktree switching with pattern matching and user selection
 */
export async function handleWorktreeSwitch(
  repoInfo: RepositoryInfo,
  pattern?: string
): Promise<void> {
  ...
  // If no pattern provided, list all worktrees for user selection
  if (!pattern) {
    if (worktrees.length === 1) {
      // Only one worktree, switch to it
      const firstWorktree = worktrees[0];
      if (firstWorktree) {
        switchToWorktree(firstWorktree);
      }
      return;
    }
    
    // Multiple worktrees, let user select
    const selectedWorktree = await promptWorktreeSelection(worktrees);
    if (selectedWorktree) {
      switchToWorktree(selectedWorktree);
    } else {
  ...
  // Pattern provided, find matching worktrees
  const matchingWorktrees = findWorktreesByPattern(worktrees, pattern);
  
  if (matchingWorktrees.length === 0) {
    throw new Error(`No worktrees found matching pattern: ${pattern}`);
  }
  
  if (matchingWorktrees.length === 1) {
    // Single match, switch to it
    const firstMatch = matchingWorktrees[0];
    if (firstMatch) {
      switchToWorktree(firstMatch);
    }
    return;
  }
  
  // Multiple matches, let user select
  const selectedWorktree = await promptWorktreeSelection(matchingWorktrees);
  if (selectedWorktree) {
    switchToWorktree(selectedWorktree);
  } else {
    throw new Error('No worktree selected.');
...
/**
 * Switches to a worktree by outputting the path for shell integration
 */
export function switchToWorktree(worktree: WorktreeInfo): void {
  // Output the path to stdout for shell integration
  // Shell scripts can capture this with: cd "$(wt switch pattern)"
  console.log(worktree.path);
}

/**
 * Prompts user to select from multiple matching worktrees
 */
export function promptWorktreeSelection(worktrees: WorktreeInfo[]): Promise<WorktreeInfo | null> {
  return new Promise((resolve) => {
    console.error('Multiple worktrees found:');
    console.error(formatWorktreeHeader());
    
    worktrees.forEach((worktree, index) => {
      const indicator = (index + 1).toString().padStart(2);
      const formattedWorktree = formatWorktree(worktree);
      console.error(`${indicator}) ${formattedWorktree.substring(2)}`); // Remove the leading indicator from formatWorktree
    });
    
    console.error('\nSelect a worktree (1-' + worktrees.length + ') or press Enter to cancel:');
    ...
    const onData = (input: string) => {
      ...
      if (isNaN(selection) || selection < 1 || selection > worktrees.length) {
        console.error('Invalid selection.');
        resolve(null);
        return;
      }
      
      resolve(worktrees[selection - 1] || null);
    };
